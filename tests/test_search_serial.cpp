...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (!searcher.search_serial(query_data.data(), query_num, ground_truth, nprobe, results, recalls, &stats)) {
        ...
        // 保存统计信息到文件
        std::string stats_file = std::string(argv[6]) + "/search_serial_statistics.txt";
