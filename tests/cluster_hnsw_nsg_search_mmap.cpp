...
bool load_cluster_data(
    ...
    std::string cluster_filename = prefix + "/cluster_data/cluster_" + std::to_string(cluster_id) + ".data";
...
SearchContext initialize_search_context(
    ...
    std::vector<float> centroids = CNNS::load_centroids(prefix + "/centroids.data", ctx.n_clusters, ctx.m, centroids_dim);
